...
export class DebugProtocolAdapter {
  ...
  private async attemptReconnect() {
    ...
    try {
      for (let i = 0; i < maxRetries; i++) {
        this.logger.log(`!!!! Attempting to reconnect after a relaunch... ${i}`);
        if (this.isDestroyed || this.connected) break;

        this.logger.log(`Reconnection attempt ${i + 1}/${maxRetries}`);
        try {
          await this.createDebugProtocolClient();
          this.logger.log('Reconnected successfully');
          break;
        } catch (e) {
          this.logger.log(`Reconnection attempt ${i + 1} failed, retrying in ${retryInterval}ms`);
  ...
  public async processTelnetOutput() {
    ...
    try {
      ...
      this.findWaitForDebuggerPrompt(settledLogs);
